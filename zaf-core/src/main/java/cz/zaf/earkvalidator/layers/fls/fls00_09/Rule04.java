...
public class Rule04 extends AipRule {
	public static final String CODE = "fls04";
	public static final String RULE_TEXT = "Lze zahájit kontroly správnosti souborů ve formátu PREMIS ve změnovém balíčku.";
	public static final String RULE_ERROR = "Nepodařilo se připravit kontrolu souborů ve formátu PREMIS.";
	public static final String RULE_SOURCE = "CZDAX-PMD0201, CZDAP-IPF0201";
			...
			if(EarkConstants.STRUCTMAP_LABEL_METADATA.equals(label)) {
				div.getADMID().forEach(admId -> {
					...
					if(MetsNS.MDTYPE_PREMIS.equals(mdRef.getMDTYPE())) {
						preparePremisValidation(mdRef, mdRef.getHref());
					}
				});
			}
		}
	...
	private void preparePremisValidation(MdRef mdRef, String relativePath) {
		...
		PremisProfile profile;
		if(isPackageInfo) {
			profile = PremisProfile.PACKAGE_INFO_CHANGE;
		} else {
			throw new ZafException(BaseCode.CHYBI_ELEMENT, "Nepodporovaný typ PREMIS, cesta" + relativePath + ".", ctx.formatMetsPosition(mdRef));
		}
