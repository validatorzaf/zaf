...
public class Rule56 extends EadRule {
    ...
    static final public String RULE_TEXT = "Hodnota atributu \"localtype\" elementu <daterange>, který je obsažený v elementu <unitdatestructured> odpovídá některé z následujících hodnot: "
            + " CONTENT"
            + " DECLARED"
            + " ORIGIN"
            + " COPY"
    ...
    static final public String RULE_ERROR = "Atribut \"localtype\" některého elementu <daterange> v elementu <unitdatestructured> má atribut \"localtype\", který obsahuje nepovolenou hodnotu.";
			...
			if (localtype != null) {
				if (!validValues.contains(localtype)) {
					throw new ZafException(BaseCode.CHYBNA_HODNOTA_ATRIBUTU,
							"Atribut localtype obsahuje nepovolenou hodnotu: " + localtype + ".",
							ctx.formatEadPosition(daterange));
				}
				ctx.markValidatedAttributeOnly(daterange, "localtype");
			}
		}
    }
