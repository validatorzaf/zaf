...
public class Rule69 extends EadRule {
    ...
    static final public String RULE_TEXT = "Každý element <ead:altformavail> obsahuje právě jeden neprázdný element <ead:p>.";
    ...
    protected void evalImpl() {
        ...
        for (Object archDescChild : archDescChildList) {
            if (archDescChild instanceof Altformavail) {
        ...
        ctx.getEadLevelIterator().iterate((c, parent) -> {
            ...
            for (Object cChild : cChildList) {
                if (cChild instanceof Altformavail) {
    ...
    private void validateMainElement(Object instanceOfObject) {
        Altformavail elementObject = (Altformavail) instanceOfObject;
        ...
        for (Object child : childList) {
            ...
            //ead:custodhist
            if (child instanceof Altformavail) {
                validateMainElement(child);
            }
        }
        if (p == null) {
            throw new ZafException(BaseCode.CHYBI_ELEMENT, "Element <ead:altformavail> neobsahuje element <ead:p>.", ctx.formatEadPosition(elementObject));
        }
    }
