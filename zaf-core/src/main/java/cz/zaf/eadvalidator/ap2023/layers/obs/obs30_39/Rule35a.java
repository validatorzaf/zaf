package cz.zaf.eadvalidator.ap2023.layers.obs.obs30_39;

import org.apache.commons.lang3.StringUtils;

import cz.zaf.common.exceptions.ZafException;
import cz.zaf.common.exceptions.codes.BaseCode;
import cz.zaf.eadvalidator.ap2023.EadRule;
import cz.zaf.schema.ead3.Archdesc;

public class Rule35a extends EadRule {

    static final public String CODE = "obs35a";
    static final public String RULE_TEXT = "Element <archdesc> má atribut \"level\" o hodnotě \"fonds\" nebo o hodnotě \"otherlevel\" s atributem otherlevel=\"balicek\"";
    static final public String RULE_ERROR = "Elementu <archdesc> chybí atribut \"level\" nebo tento atribut neobsahuje hodnotu \"fonds\" nebo \"otherlevel\".";
    static final public String RULE_SOURCE = "Část 3.1 profilu EAD3 MV ČR";

    public Rule35a() {
        super(CODE, RULE_TEXT, RULE_ERROR, RULE_SOURCE);
    }

    @Override
    protected void evalImpl() {
        Archdesc archdesc = ctx.getEad().getArchdesc();
        String level = archdesc.getLevel();
        if (!validate(level)) {
            throw new ZafException(BaseCode.CHYBNA_HODNOTA_ATRIBUTU, "Atribut level elementu archdesc není vyplněn.", ctx.formatEadPosition(archdesc));
        }
        if(StringUtils.equals("otherlevel", level)){
            String otherlevel = archdesc.getOtherlevel();
            if(StringUtils.isEmpty(otherlevel)){
                throw new ZafException(BaseCode.CHYBNA_HODNOTA_ATRIBUTU, "Atribut otherlevel elementu archdesc není vyplněn.", ctx.formatEadPosition(archdesc));
            }
            if(!StringUtils.equals("balicek", otherlevel)){
                throw new ZafException(BaseCode.CHYBNA_HODNOTA_ATRIBUTU, "Atribut otherlevel elementu archdesc neobsahuje očekávanou hodnotu \"balicek\", ale hodnotu: " + otherlevel + ".", ctx.formatEadPosition(archdesc));
            }
        }
    }
    
    private boolean validate(String level){
        return StringUtils.equals("fonds", level) || StringUtils.equals("otherlevel", level);
    }

}
