...
public class Pravidlo63c extends K06PravidloBase {

    static final public String OBS63C = "obs63c";

    public Pravidlo63c() {
        super(OBS63C,
                "Pokud je základní entitou díl (nsesss:Dil), potom v hierarchii dětských elementů nsesss:EvidencniUdaje, nsesss:Uzavreni je hodnota roku v elementu nsesss:Datum nižší než aktuální rok.",
                "Chybí odůvodnění vyřízení jiným způsobem.",
                "§ 15 odst. 3 vyhlášky č. 259/2012 Sb.");
    }

    //OBSAHOVÁ č.63c Pokud je základní entitou díl (nsesss:Dil), potom v hierarchii dětských elementů nsesss:EvidencniUdaje, nsesss:Uzavreni je hodnota 
    ...
    protected void kontrola() {
        ...
        if (!CollectionUtils.isEmpty(zakladniEntity)) {
            for (Element elZaklEntita : zakladniEntity) {
                ...
                if (zaklEntName.equals(NsesssV4.DIL)) {
                    Element elDatum = ValuesGetter.getXChild(elZaklEntita, NsesssV4.EVIDENCNI_UDAJE, NsesssV4.UZAVRENI, NsesssV4.DATUM);
                    if (elDatum == null) {
